...
const FFUserDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      Array.isArray(userProfile.data.user.phoneNumbers)
    ) {
      setAdditionalPhones(
        userProfile.data.user.phoneNumbers.map(
          (p: { value: string; country: string }) => ({
            value: p.value,
            country: p.country || "",
          })
        )
    ...
    if (userProfile && userProfile.data && userProfile.data.user) {
      ...
      if (Array.isArray(emails)) {
        setAdditionalEmails(
          emails.map((e: any) => (typeof e === "string" ? { value: e } : e))
        );
      } else if (typeof emails === "string" && emails.trim()) {
        setAdditionalEmails(
          emails.split(",").map((e: string) => ({ value: e.trim() }))
        );
      } else {
        setAdditionalEmails([]);
      }
    ...
    if (
      ...
      userProfile.data &&
      userProfile.data.user &&
      Array.isArray(userProfile.data.user.socialLinks)
    ) {
      const customLinks = userProfile.data.user.socialLinks.filter(
        (l: { platform: string; url: string }) =>
          !["linkedin", "x", "instagram"].includes(l.platform)
      );
  ...
  const getStatusBadge = (status: string) => {
    ...
    const config =
      statusConfig[status as keyof typeof statusConfig] || statusConfig.invited;
  ...
  if (userProfile) {
    const profile =
      editMode && editedProfile ? editedProfile : userProfile.data.user;
    ...
    const standardLinks =
      profile?.socialLinks?.filter((l: { platform: string; url: string }) =>
        ["linkedin", "x", "instagram"].includes(l.platform)
      ) || [];
    ...
    const handleCustomLinkChange = (
      ...
      setEditedProfile((prev) => {
        if (!prev) return prev;
        const links = prev.socialLinks ? [...prev.socialLinks] : [];
        const standard = links.filter((l) =>
          ["linkedin", "x", "instagram"].includes(l.platform)
        );
        const updatedCustomLinks = [...customSocialLinks];
        updatedCustomLinks[idx] = {
          ...updatedCustomLinks[idx],
          [field]: value,
        };
    ...
    const removeCustomSocialLink = (index: number) => {
      ...
      setEditedProfile((prev) => {
        ...
        const updatedCustomLinks = customSocialLinks.filter(
          (_, i) => i !== index
        );
        return { ...prev, socialLinks: [...standard, ...updatedCustomLinks] };
      });
        ...
        setEditedProfile((prev) =>
          prev
            ? {
                ...
                phoneNumber: {
                  value: "",
                  country: prev.phoneNumber?.country || "",
                },
              }
            : prev
        );
    ...
    const handleSave = async () => {
      ...
      if (
        ...
        !editedProfile.phoneNumber.value.trim()
      ) {
      ...
      if (editedProfile?.socialLinks) {
        editedProfile.socialLinks.forEach((link: any) => {
          if (link.url && link.url.trim()) {
            ...
            if (!/^https?:\/\//i.test(url)) {
              errors[`socialLink${link.platform}`] =
                "URL must start with http:// or https://";
            } else if (!/^https?:\/\/[^\/\s]+/i.test(url)) {
              errors[`socialLink${link.platform}`] =
                "Please enter a complete URL with domain";
            }
          }
        });
      }
      ...
      if (customSocialLinks && Array.isArray(customSocialLinks)) {
        customSocialLinks.forEach((link: any, idx: number) => {
          ...
          if (!link.platform || !link.platform.trim()) {
            errors[`customSocialLinkPlatform${idx}`] =
          ...
          if (
            ...
            const url = link.url.trim();
            if (!/^https?:\/\//i.test(url)) {
              errors[`customSocialLinkUrl${idx}`] =
                "URL must start with http:// or https://";
            } else if (!/^https?:\/\/[^\/\s]+/i.test(url)) {
              errors[`customSocialLinkUrl${idx}`] =
                "Please enter a complete URL with domain";
      ...
      const updatedData = {
        ...
        socialLinks: [
          ...(editedProfile?.socialLinks?.filter((l: any) =>
            ["linkedin", "x", "instagram"].includes(l.platform)
          ) || []),
          ...customSocialLinks.filter(
    ...
    const performSave = async (updatedData: any, userId: string) => {
      ...
      try {
        ...
        if (response.data.user) {
          setAdditionalPhones(
            Array.isArray(response.data.user.phoneNumbers)
              ? response.data.user.phoneNumbers.map(
                  (p: { value: string; country: string }) => ({
                    value: p.value,
                    country: p.country || "",
                  })
    ...
    const handleMarkAsConfigured = async () => {
      try {
        ...
        const invitationId =
          invitationData._id || userProfile.data.invitation?._id;
        if (!invitationId) {
        ...
        const response = await apiService.put(
          `/invitation/${invitationId}/nfc-status`,
          {
        ...
        const errorMsg =
          error?.response?.data?.message || "Failed to update NFC status";
        toast.error(errorMsg);
      }
    };
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {editMode ? (
                    ...
                    (() => {
                      const emails = profile?.additionalEmails
                      ...
                      if (emails.length === 0) {
                        return (
                          <input
                            ...
                            disabled={!editMode}
                            onChange={(e) =>
                              handleChange("additionalEmails", e.target.value)
                            }
                          />
                        );
                      }
                        ...
                        >
                          <div className="flex-1">
                            <SafePhoneInput
                              ...
                              onChange={(value) => {
                                const country = getSafeCountryCode(
                                  phone.country || "AE"
                                );
                    ...
                    <input
                      ...
                      value={
                        editMode
                          ...
                          : displayValue(profile?.address?.zipCode)
                      }
                      disabled={!editMode}
                      onChange={(e) =>
                      ...
                      <Select
                        ...
                        value={
                          profile?.address?.country
                            ? countries.find(
                                (c) => c.label === profile?.address?.country
                              )
                            : null
                        }
                        ...
                        styles={{
                          ...
                          control: (provided: any, state: any) => ({
                            ...
                            border: validationErrors.addressCountry
                              ? "1px solid #ef4444"
                              : selectStyles.control(provided, state).border,
                          }),
                        }}
                        isSearchable={true}
                        isClearable={true}
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                {customSocialLinks.length > 0 ? (
                  customSocialLinks.map(
                    (l: { platform: string; url: string }, idx: number) => (
